    ...
    public static void main (String[] args) 
        ...
        try (var cache = Executors.newCachedThreadPool();){
            UserDatabase.Factory dbFactory =  
                    (UserDatabase.Factory) Class.forName(DEFAULT_DB_FACTORY).getDeclaredConstructor().newInstance();
 

            UserDatabase userDatabase = dbFactory.createDB();

            UserInterface.Factory uiFactory = 
                    (UserInterface.Factory) Class.forName(DEFAULT_UI_FACTORY).getDeclaredConstructor().newInstance();
