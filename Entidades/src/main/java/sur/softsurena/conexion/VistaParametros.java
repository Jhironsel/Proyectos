...
public final class VistaParametros extends javax.swing.JFrame {

    private static final long serialVersionUID = 1L;

    private final Properties propiedades;
    private final File filePropertie = new File("../properties/propiedades.properties");

    public VistaParametros() {
        initComponents();
        propiedades = new Properties();

        try (var fr = new FileReader(filePropertie)){
            propiedades.load(fr);
        } catch (FileNotFoundException ex) {
            try {
                var file = filePropertie.createNewFile();
                if(file){
                    JOptionPane.showMessageDialog(
                            null, 
                            "Vuelva a iniciar la aplicacion", 
                            "", 
                            JOptionPane.ERROR_MESSAGE
                    );
                    throw new ExceptionInInitializerError("Error al inicializar la ventana.");
                }
            } catch (IOException ex1) {
                LOG.log(
                        Level.SEVERE,
                        "El archivo no fue creado ../properties/propiedades.properties",
                        ex
                );
            }
            LOG.log(
            ...
            LOG.log(
                    Level.SEVERE,
                    "Error de entrada o salida del archivo propiedades.",
                    ex
            );
        }
    }
    ...
    private void escribirParametros() {
        ...
        try (var fr = new FileWriter(filePropertie)){
            propiedades.store(fr, "Parametros del Servidor actual");
        } catch (IOException ex) {
            LOG.log(
                    Level.SEVERE, 
