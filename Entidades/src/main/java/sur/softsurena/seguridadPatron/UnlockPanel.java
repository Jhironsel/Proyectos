...
public final class UnlockPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private final ArrayList<FigurePattern> pattern = new ArrayList<>();
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        figurePatternArr.stream().filter(
                (p) -> (FigurePattern.insideArea(p.getArea(), e.getPoint()))
        ).forEach(
                (p) -> {
    ...
    /**
     * metodo para comprobar que patron de desbloqueo es correcto de ser asi ->
     * abre nuevo formulario caso contrario -> muestra mensaje de error
     */
