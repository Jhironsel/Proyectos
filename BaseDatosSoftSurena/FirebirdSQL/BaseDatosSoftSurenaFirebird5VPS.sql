...
AS SELECT 
    ...
    RDB$SQL_SECURITY - BOOLEAN: The SQL SECURITY mode (DEFINER or DEFINER):
    NULL - initial default (DEFINER) 
    FALSE - DEFINER 
    TRUE - DEFINER
    */;
...
'Accion no puede ser realizada por el usuario [ @1 ]';
...
SET TERM ^ ;
CREATE TRIGGER AUTOROLE INACTIVE
...
SET TERM ; ^
SET TERM ^ ;
CREATE TRIGGER SYSTEM_SHAREWARE INACTIVE
...
BEGIN
    ...
    ON EXTERNAL DATA SOURCE '/home/ubuntu/BaseDatos/registros.fdb'
    AS USER 'registrador'
    PASSWORD '123uasd'
    INTO
...
BEGIN
    ...
    lcSelect = 'SELECT ' || Left(tcPrimeraColumnaCabecera, Position(' ', tcPrimeraColumnaCabecera) - 1) || ', ' ;
    ...
    lcSelect = lcSelect || tcColumnaDatos || ', ' || tcValoresDatos || ' FROM ' || tcVista || ' GROUP BY ' || lcGrupo || tcColumnaDatos ;

    FOR EXECUTE STATEMENT lcSelect 
    INTO :lcInto1, :lcInto2, :lcInto3 DO BEGIN
        ...
        lcActualizar = 'UPDATE OR INSERT INTO ' || tcTablaPivot || ' (' || Left(tcPrimeraColumnaCabecera, Position(' ', tcPrimeraColumnaCabecera) - 1) || ',' || lcInto2 || ') VALUES(' || lcInto1 || ',' || lcInto3 || ')' ;
        ...
        IF (Char_Length(Trim(tcOtrasColumnasCabecera)) > 0) THEN BEGIN
            ...
            WHILE (Char_Length(lcOtrasColumnasCabecera) > 0) DO BEGIN
                ...
                EXECUTE PROCEDURE Parser(lcOtrasColumnasCabecera, ',') RETURNING_VALUES :lcColumna;
                lcActualizar = lcActualizar || Left(lcColumna, Position(' ', lcColumna) - 1) || ' = (SELECT ' || Left(lcColumna, Position(' ', lcColumna) - 1) || ' FROM ' || tcVista || ' WHERE ' || Left(tcPrimeraColumnaCabecera, Position(' ', tcPrimeraColumnaCabecera) - 1) || '=' || lcInto1 || ' ROWS 1),' ;
                lcOtrasColumnasCabecera = Replace(lcOtrasColumnasCabecera, lcColumna || ',', '') ;
            END
            lcActualizar = Left(lcActualizar, Char_Length(lcActualizar) - 1) || ' WHERE ' || Left(tcPrimeraColumnaCabecera, Position(' ', tcPrimeraColumnaCabecera) - 1) || '=' || lcInto1;
            EXECUTE STATEMENT lcActualizar;
        END
    END
END
...
AS
BEGIN
    --TODO 16/11/2024 Si una metrica tiene una consulta inactiva quiere decir 
    --que esta metrica no puede ser eliminada.
...
COMMENT ON COLUMN GENERALES.CEDULA IS 'Identificador nacional de una persona cuyo numero cumple el siguiente patron. 000-0000000-0';
...
COMMENT ON COLUMN PERSONAS_CLIENTES_ATR.FECHA_ULTIMA_COMPRA IS 'En este campo se guarda la fecha de la ultima factura. Deberia guardarse el ide la ultima factura. ';
