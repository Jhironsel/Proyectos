...
CREATE PROCEDURE SP_D_D_MOTIVO_CONSULTA (
    I_ID_CONSULTA TYPE OF COLUMN V_D_MOTIVO_CONSULTA.IDCONSULTA,
    I_ID_M_CONSULTA TYPE OF COLUMN V_D_MOTIVO_CONSULTA.IDMCONSULTA )
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
CREATE PROCEDURE SP_I_CATEGORIA (
    ...
    IMAGEN_TEXTO TYPE OF COLUMN CATEGORIAS.IMAGEN_TEXTO,
    ESTADO TYPE OF COLUMN CATEGORIAS.ESTADO )
RETURNS (
    V_ID TYPE OF COLUMN CATEGORIAS.ID )
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
CREATE PROCEDURE SP_I_ENTRADA_PRODUCTO (
    ID_PROVEEDOR TYPE OF COLUMN V_PERSONAS_PROVEEDORES.ID,
    COD_FACTURA TYPE OF COLUMN V_M_ENTRADA_PRODUCTOS.COD_FACTURA,
    LINEA TYPE OF COLUMN V_D_ENTRADA_PRODUCTO.LINEA,
    ID_PRODUCTO TYPE OF COLUMN V_PRODUCTOS.ID,
    ENTRADA TYPE OF COLUMN V_D_ENTRADA_PRODUCTO.ENTRADA,
    FECHA_VECIMIENTO TYPE OF COLUMN V_D_ENTRADA_PRODUCTO.FECHA_VECIMIENTO )
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
CREATE PROCEDURE SP_I_USUARIO (
    ...
    I_ESTADO TYPE OF COLUMN VS_USUARIOS.ESTADO,
    I_ADMINISTRADOR TYPE OF COLUMN VS_USUARIOS.ADMINISTRADOR,
    I_DESCRIPCION TYPE OF COLUMN VS_USUARIOS.DESCRIPCION,
...
CREATE VIEW GET_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, 
    ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, FECHA_HORA, TOTAL, 
    EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP, USER_NAME, PNOMBRE, SNOMBRE, 
    APELLIDOS)
AS /*
...
*/;
CREATE VIEW GET_PACIENTES (ID, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, 
    FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, ESTADO, 
    PESO_NACIMIENTO_KG, ALTURA, PERIMETRO_CEFALICO, CESAREA, TIEMPO_GESTACION, 
    MASA_CEFALICA)
AS /*
...
CREATE VIEW GET_PRIVILEGIOS (USER_NAME, CEDENTE, PRIVILEGIO, OPCION_PERMISO, 
    NOMBRE_RELACION, NOMBRE_CAMPO, TIPO_USUARIO, TIPO_OBJECTO, DESCRIPCION)
AS /*
...
*/;
CREATE VIEW GET_PRODUCTOS (ID, ID_CATEGORIA, DESC_CATEGORIA, IMAGEN_CATEGORIA, 
    CODIGO, DESCRIPCION, EXISTENCIA, IMAGEN_PRODUCTO, NOTA, FECHA_CREACION, 
    ESTADO)
AS /*
...
CREATE VIEW GET_PROVEEDORES (ID, PERSONA, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, 
    ESTADO, CEDULA, ID_PERSONA, ID_PROVINCIA, ID_MUNICIPIO, DIRECCION, CODIGO)
AS /*
...
FROM V_TANDAS t;
CREATE VIEW GET_TURNOS (ID, ID_ALMACEN, TURNO_USUARIO, FECHA_HORA_INICIO, 
    FECHA_HORA_FINAL, ESTADO, MONTO_FACTURADO, MONTO_DEVUELTO, MONTO_EFECTIVO, 
...
CREATE VIEW VS_CATEGORIAS (ID, DESCRIPCION, IMAGEN_TEXTO, FECHA_CREACION, 
    ESTADO, USER_NAME)
AS /*
...
CREATE VIEW VS_PRIVILEGIO (USUARIO, GRANTOR, ABRV_PRIVILEGIO, NOMBRE_ABREV, 
    ...
    ID_TIPO_USUARIO, TIPO_USUARIO, ID_TIPO_OBJ, TIPO_OBJ)
AS /*
...
FROM D_DEUDAS_PAGAS 
WHERE ID >= 0;
CREATE VIEW V_D_ENTRADA_PRODUCTO (ID_M_ENTRADA_PRODUCTO, LINEA, ID_PRODUCTO, 
    ENTRADA, COSTO, PRECIO, FECHA_VECIMIENTO, OBSERVACION)
AS /*
...
FROM HUELLAS
WHERE ID >= 0;
CREATE VIEW V_INSCRIPCION (ID, ID_ESTUDIANTE, ID_TANDA, PAGO, 
...
CREATE VIEW V_METRICAS (ID, IDCONSULTA, FECHA, PESOKG, ESTATURAMETRO, ESCEFALO, 
    NF_DETECT, HALLAZGOS_POS, ID_DIAG, TX, COMPLEMENTO, IMAGEN_TEXTO, USER_NAME)
AS /*
...
FROM M_DEUDAS
WHERE ID >= 0;
CREATE VIEW V_M_ENTRADA_PRODUCTOS (ID, ID_PROVEDOR, ID_ALMACEN, COD_FACTURA, 
...
CREATE VIEW V_M_FACTURAS (ID, ID_CLIENTE, ID_CONTACTOS_TEL, 
    ID_CONTACTOS_DIRECCIONES, ID_CONTACTOS_EMAIL, ID_TURNO, FECHA_HORA, TOTAL, 
    EFECTIVO, ESTADO_FACTURA, NOMBRE_TEMP, USER_NAME)
AS /*
...
WHERE ID >= 0;
CREATE VIEW V_PERSONAS_PACIENTES_ATR (ID, PESO_NACIMIENTO_KG, ALTURA, 
    PERIMETRO_CEFALICO, CESAREA, TIEMPO_GESTACION, MASA_CEFALICA, USER_NAME)
AS /*
...
CREATE VIEW V_PRODUCTOS (ID, ID_CATEGORIA, CODIGO, DESCRIPCION, EXISTENCIA, 
    IMAGEN_TEXTO, NOTA, FECHA_CREACION, ESTADO, USER_NAME, ROL)
AS /*
     Inicio
*/
SELECT 
...
*/;
CREATE VIEW V_TURNOS (ID, ID_ALMACEN, TURNO_USUARIO, FECHA_HORA_INICIO, 
    FECHA_HORA_FINAL, ESTADO, MONTO_FACTURADO, MONTO_DEVUELTO, MONTO_EFECTIVO, 
...
CREATE VIEW V_T_E_S_SYS (ID, NOMBRE_EMPRESA, FCHI, FCHA, FCHV, DIAS_RESTANTES, 
    TELEFONO, DIRECCION, MENSAJE_FOOTER, LOGO)
AS /*
...
DECLARE VARIABLE tmp DECIMAL(18,0);
BEGIN
  IF (NEW.ID_M_ENTRADA_PRODUCTO IS NULL OR NEW.ID_M_ENTRADA_PRODUCTO = -1) THEN
...
BEGIN
    ...
    FOR SELECT ID FROM V_CONSULTAS_APROBADAS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONSULTA_APROBADAS(:v_id);
    END
    
    FOR SELECT ID FROM V_ANTECEDENTES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_ANTECEDENTE(:v_id);
    END
    
    FOR SELECT ID FROM V_CONSULTAS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONSULTA(:v_id);
    END
    
    FOR SELECT ID FROM V_CONTROL_CONSULTA INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONTROL_CONSULTA(:v_id);
    END
    
        
    FOR SELECT ID FROM V_ALMACENES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_ALMACEN(:v_id);
    END

    FOR SELECT ID_PERSONA FROM V_ASEGURADOS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_ASEGURADO(:v_id);
    END
    
    FOR SELECT ID FROM V_ARS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_ARS(:v_id);
    END
    
    FOR SELECT ID FROM V_CARTONES_BINGO INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CARTON_BINGO(:v_id);
    END
    
    FOR SELECT ID FROM V_CATEGORIAS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CATEGORIA(:v_id);
    END
    
    FOR SELECT ID FROM V_CONTACTOS_DIRECCIONES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONTACTO_DIRECCION(:v_id);
    END
    
    FOR SELECT ID FROM V_CONTACTOS_EMAIL INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONTACTO_EMAIL(:v_id);
    END
    
    FOR SELECT ID FROM V_CONTACTOS_TEL INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_CONTACTO_TEL(:v_id);
    END
    
    FOR SELECT ID FROM V_D_ANALISIS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_D_ANALISIS(:v_id);
    END
    
    FOR SELECT ID FROM V_M_DEUDAS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_M_DEUDAS(:v_id);
    END
        
    FOR SELECT ID FROM V_D_DEUDAS_PAGAS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_D_DEUDAS_PAGAS(:v_id);
    END
    
    FOR SELECT ID FROM V_PRODUCTOS INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_PRODUCTO(:v_id);
    END
    
    FOR SELECT ID_PERSONA FROM V_GENERALES WHERE ID_PERSONA != 0 INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_GENERAL(:v_id);
    END
    
    --TODO Agregar un ID a esta vista.
    FOR SELECT d.IDCONSULTA, d.IDMCONSULTA FROM V_D_MOTIVO_CONSULTA d INTO v_id, v_id2 DO BEGIN
        EXECUTE PROCEDURE SP_D_D_MOTIVO_CONSULTA(:v_id, :v_id2);
    END
        
    
    --ORDEN de las Facturas
    FOR SELECT ID FROM V_M_FACTURAS INTO v_id DO BEGIN
    ...
    FOR SELECT ID FROM V_PERSONAS_PACIENTES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_PERSONA_PACIENTE(:v_id);
    END
    
    FOR SELECT ID FROM V_PERSONAS_ESTUDIANTES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_PERSONA_ESTUDIANTE(:v_id);
    END
    
    FOR SELECT ID FROM V_PERSONAS_PROVEEDORES INTO v_id DO BEGIN
        EXECUTE PROCEDURE SP_D_PERSONA_PROOVEDOR(:v_id);
    END
    
    FOR SELECT ID FROM V_PERSONAS_CLIENTES WHERE ID != 0 INTO v_id DO BEGIN
...
BEGIN
    ...
    DELETE FROM V_D_MOTIVO_CONSULTA 
    WHERE IDCONSULTA = :I_ID_CONSULTA AND IDMCONSULTA = :I_ID_M_CONSULTA;
END
^
SET TERM ; ^
...
AS
BEGIN
    --TODO VALIDAR QUE DESCUENTO NO ESTE POR DEBAJO DE 0 NI POR ENCIMA DE 100
...
BEGIN
    ...
    INSERT INTO D_MOTIVO_CONSULTA (IDCONSULTA, IDMCONSULTA)
    VALUES (:I_ID_CONSULTA, :I_ID_M_CONSULTA);
END
^
SET TERM ; ^
...
BEGIN
     IF((SELECT (1) 
            FROM V_CATEGORIAS c 
            WHERE c.ID = :ID_CATEGORIA AND c.ESTADO IS FALSE) = 1)THEN
        EXCEPTION E_CATEGORIAS_INACTIVA;
        
     INSERT INTO V_PRODUCTOS (ID_CATEGORIA, CODIGO, DESCRIPCION, IMAGEN_TEXTO, NOTA, ESTADO)
     VALUES (:ID_CATEGORIA, :CODIGO, :DESCRIPCION, :IMAGEN_TEXTO, :NOTA, :ESTADO)
     RETURNING ID 
     INTO O_ID;
...
AS
DECLARE VARIABLE V_ESTADO TYPE OF COLUMN  CATEGORIAS.ESTADO;
BEGIN
...
ALTER TABLE CONSULTAS ADD CONSTRAINT FK_CONSULTAS_1
  FOREIGN KEY (ID_CONTROL_CONSULTA) REFERENCES CONTROL_CONSULTA (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
ALTER TABLE D_MOTIVO_CONSULTA ADD CONSTRAINT FK_D_MOTIVO_CONSULTA_0
  FOREIGN KEY (IDMCONSULTA) REFERENCES T_MOTIVO_CONSULTA (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
...
GRANT CAJERO TO JHIRONSEL WITH ADMIN OPTION GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_T_E_S_SYS TO ROLE CAJERO WITH GRANT OPTION GRANTED BY SYSDBA;
