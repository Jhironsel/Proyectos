...
CREATE PROCEDURE SP_D_PERSONA_ESTUDIANTE (
    V_ID TYPE OF COLUMN PERSONAS_ESTUDIANTES.ID )
SQL SECURITY DEFINER
AS 
BEGIN SUSPEND; 
END^
...
AS SELECT 
    r.ID, 
    g.CEDULA, g.ID_TIPO_SANGRE, g.ESTADO_CIVIL
...
BEGIN
     v_estado = (SELECT ESTADO FROM V_ARS WHERE ID = :V_ID);
     
     DELETE FROM V_ARS WHERE ID = :V_ID;
...
BEGIN
    ...
    --No se permite operar con la persona generica.
     IF(:ID_PERSONA <= 0)THEN
            EXCEPTION E_CLIENTE_GENERICO USING(:ID_PERSONA);
