...
CREATE SEQUENCE SEQ_CONSULTAS_APROBADAS_ID START WITH 1 INCREMENT BY 1;
...
*/;
CREATE VIEW GET_PACIENTES (ID, PNOMBRE, SNOMBRE, APELLIDOS, SEXO, FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_HORA_ULTIMO_UPDATE, ESTADO, PESO_NACIMIENTO_KG, 
...
SELECT 
     ...
     ESTADO
FROM CARTONES_BINGO;
CREATE VIEW V_CATEGORIAS (ID, DESCRIPCION, FECHA_CREACION, ESTADO, USER_NAME, IMAGEN_TEXTO)
AS SELECT 
    ID, 
    DESCRIPCION, 
    FECHA_CREACION, 
...
CREATE VIEW V_PERSONAS_PACIENTES_ATR (ID, PESO_NACIMIENTO_KG, ALTURA, PERIMETRO_CEFALICO, CESAREA, TIEMPO_GESTACION, MASA_CEFALICA, USER_NAME)
AS /*
...
CREATE VIEW V_T_E_S_SYS (ID, NOMBRE_EMPRESA, FCHI, FCHA, FCHV, DIAS_RESTANTES, TELEFONO, DIRECCION, MENSAJE_FOOTER, PATH_LOGO, LOGO, URLDB)
AS /*
...
'El codigo de la provincia no est� definido en el sistema.';
...
COMMENT ON PROCEDURE ACTUALIZAR_TABLA_PIVOT IS 'Este procedimiento en Firebird parece estar dise�ado para actualizar una tabla pivot en funci�n de los datos de una vista y algunas otras columnas especificadas. Aqu� est� el desglose de lo que hace:

1. **Declaraci�n de Variables**: Comienza declarando varias variables que se utilizar�n en el procedimiento.

2. **Construcci�n de Consulta SELECT**: El procedimiento construye una consulta SELECT din�mica utilizando los par�metros de entrada, 
	como `TCVISTA`, `TCPRIMERACOLUMNACABECERA`, `TCOTRASCOLUMNASCABECERA`, `TCCOLUMNADATOS`, y `TCVALORESDATOS`. La consulta se construye para agrupar datos en funci�n de las columnas especificadas.

3. **Ejecuci�n de la Consulta SELECT**: Luego, se ejecuta la consulta SELECT construida y los resultados se almacenan en las variables `lcInto1`, `lcInto2`, y `lcInto3`.

4. **Actualizaci�n o Inserci�n de Datos en la Tabla Pivot**: Se realiza una serie de manipulaciones en las variables y se construye una consulta de actualizaci�n o inserci�n (`UPDATE OR INSERT`) 
	en la tabla pivot (`tcTablaPivot`). Esto se hace en funci�n de los resultados de la consulta SELECT. Los datos de la columna de la cabecera se tratan de acuerdo con su tipo de datos y se reemplazan 
	algunos caracteres especiales en el nombre de la columna.

5. **Actualizaci�n de Otras Columnas de la Cabecera**: Si `tcOtrasColumnasCabecera` contiene datos adicionales, el procedimiento tambi�n actualiza estas columnas en la tabla pivot. Para cada columna adicional, 
	se ejecuta una subconsulta para obtener el valor correspondiente de la vista y se actualiza en la tabla pivot.

6. **Concesi�n de Permiso**: Finalmente, se otorgan permisos de ejecuci�n en el procedimiento a los roles `RDB$ADMIN` y `SYSDBA` con la opci�n de otorgar permisos.

En resumen, este procedimiento se utiliza para mantener actualizada una tabla pivot en funci�n de los datos de una vista y otras columnas especificadas. Realiza operaciones de inserci�n y actualizaci�n en la 
	tabla pivot seg�n los resultados de la consulta SELECT. Tambi�n permite otorgar permisos de ejecuci�n a roles espec�ficos.';
SET TERM ^ ;
...
BEGIN
   ...
   INTO 
      ...
      :lcInto3
   DO BEGIN
      -- Aqu� se insertan o actualizan la primera columna y los datos agrupados
      ...
      IF (Char_Length(Trim(tcOtrasColumnasCabecera)) > 0) THEN BEGIN
        ...
        lcActualizar = 'UPDATE ' || tcTablaPivot || ' SET ' ;
         WHILE (Char_Length(lcOtrasColumnasCabecera) > 0) DO BEGIN
            -- Aqu� se actualizan todas las dem�s columnas de la cabecera
            EXECUTE PROCEDURE Parser(lcOtrasColumnasCabecera, ',') RETURNING_VALUES :lcColumna;
...
BEGIN
    ...
    --Consultamos si la factura est� creada.
...
BEGIN
     ...
     */
     ...
     /*
          La siguiente validaci�n permite saber si es un usuario tiene el rol de cajero. 
     */
     IF((SELECT DISTINCT (1) 
          FROM GET_CAJEROS 
...
COMMENT ON PROCEDURE PARSER IS 'Su misi�n es extraer un subtexto dentro de un texto.';
SET TERM ^ ;
...
AS
BEGIN
     --TODO REALIZAR VALIDACION PARA eliminar un estudiante.
     IF(:ID = 0)THEN
        EXCEPTION E_DELETE_GENERICO;
...
BEGIN
     /*
          * Validaciones del SP la cual debe validar que:
          ...
          un dia, en una semana, en un mes y en un a�o para el sistema analitico, cantidad de facturas por estado.
     */
     ...
     IF((SELECT 1 
        ...
        IF((SELECT 1 
            ...
            WHERE ID = :ID_CLIENTE) = 1)THEN BEGIN 
                EXCEPTION E_CLIENTE_NO_ENCONTRADO 'La persona no est� registrada como cliente.';
        END ELSE BEGIN 
...
BEGIN 
   ftcNuevoTexto = '';     -- El texto que se devolver�
   lnI = 1;
   lnJ = 1;
   lnK = 1;
 
   WHILE (:lnI <= CHAR_LENGTH(:tcTexto)) DO BEGIN
      lcCaracter    = SUBSTRING(:tcTexto FROM lnI FOR 1);     -- Obtiene el caracter que est� en la posici�n lnI
      
      lnAscii       = ASCII_VAL(:lcCaracter);                  -- Halla el c�digo ASCII del caracter
...
*/
     ...
     -- No se encontr� la tabla en RECCOUNT, por lo tanto hay que agregarla
...
ALTER TABLE CONSULTAS_APROBADAS ADD CONSTRAINT FK_CONSULTAS_APROBADAS_0
  FOREIGN KEY (ID) REFERENCES CONSULTAS (ID) ON UPDATE NO ACTION ON DELETE NO ACTION;
COMMENT ON TABLE CONSULTAS_APROBADAS IS 'AQUI SE ALMACENAN LAS  CONSULTAS QUE HAYAN SIDO APROVADAS.
';
...
COMMENT ON COLUMN CONTROL_CONSULTA.CANTIDADPACIENTE IS 'Cantidad de paciente que este control admiten en un dia de consulta en la semana.';
...
COMMENT ON COLUMN HUELLAS.IMAGEN_TEXTO IS 'Archivo o imagen del dedo que haya sido escaneado.';
...
COMMENT ON COLUMN TANDAS.ANNO_INICIAL IS 'Es el a�o que se piensa impartir la docencia al grupo.';
COMMENT ON COLUMN TANDAS.ANNO_FINAL IS 'Es el a�o que se piensa en termina el curso registrado.';
...
COMMENT ON COLUMN T_GUIA_VIGILANCIA_DESARROLLO.ID IS 'Identificador para indentificar las caracteristicas del ni�o a su edad.';
COMMENT ON COLUMN T_GUIA_VIGILANCIA_DESARROLLO.EDAD IS 'Las edades que registran aqui son en meses';
COMMENT ON COLUMN T_GUIA_VIGILANCIA_DESARROLLO.CARACT_DESARR_EVALUAR IS 'Describe el comportamiento de ni�o a su edad.';
...
COMMENT ON TRIGGER M_FACTURAS_BD IS 'Este Trigger no permite que las facturas nulas no sean eliminadas. ';
GRANT CAJERO TO JHIRONSEL GRANTED BY SYSDBA;
GRANT CAJERO TO JHIRONSEL2 GRANTED BY SYSDBA;
GRANT CAJERO TO PRUEBA GRANTED BY SYSDBA;
...
GRANT SECRETARIA TO JHIRONSEL2 GRANTED BY SYSDBA;
...
GRANT EXECUTE
 ON PROCEDURE ADMIN_GET_ID_FACTURA_NUEVA TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE ADMIN_SET_ROLE TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE PERM_BASICOS TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE PERM_CREAR_FACTURAS TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_D_CONSULTA_APROBADAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_D_CONSULTA_APROBADAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_I_CONSULTA_APROBADAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_I_CONSULTA_APROBADAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_S_GET_TURNOS TO ROLE CAJERO GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT EXECUTE
 ON PROCEDURE SP_U_CONSULTA_APROBADAS TO ROLE RDB$ADMIN WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;

GRANT EXECUTE
 ON PROCEDURE SP_U_CONSULTA_APROBADAS TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA                                                                                                                                                                                                                                                      ;
...
GRANT SELECT
 ON GET_M_FACTURAS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON GET_PERSONA_CLIENTES TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON GET_ROL TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON GET_TURNOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON VS_PRIVILEGIO TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_CATEGORIAS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_FOTO_PRODUCTO TO ROLE CAJERO GRANTED BY SYSDBA;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON V_FOTO_PRODUCTO TO  SYSDBA WITH GRANT OPTION GRANTED BY SYSDBA;

GRANT SELECT
 ON V_GENERALES TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_M_FACTURAS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_PERSONAS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_PERSONAS_CLIENTES TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_PRECIOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_PRODUCTOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_TURNOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_T_DISTRITOS_MUNICIPALES TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_T_E_S_SYS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_T_MUNICIPIOS TO ROLE CAJERO GRANTED BY SYSDBA;
...
GRANT SELECT
 ON V_T_PROVINCIAS TO ROLE CAJERO GRANTED BY SYSDBA;
