...
public class frmBusquedaCliente extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        setFont(new java.awt.Font("FreeMono", 0, 24)); // NOI18N
        setResizable(false);
    ...
    private void llenarTabla() {
        ...
        if (txtCriterio.getText().equals("")) {
            sql = "select idCliente, nombres, apellidos "
                    + "from tabla_clientes "
                    + "where estado order by 1";
        } else {
            if (rbtNombes.isSelected()) {
                sql = "select idCliente, nombres, apellidos "
                        + "from tabla_clientes "
                        + "where nombres like '" + txtCriterio.getText() + "%' and estado order by 1";
            }
            if (rbtApellidos.isSelected()) {
                sql = "select idCliente, nombres, apellidos "
                        + "from tabla_clientes "
                        + "where apellidos like '" + txtCriterio.getText() + "%' and estado order by 1";
            }
            if (rbtID.isSelected()) {
                sql = "select idCliente, nombres, apellidos "
                        + "from tabla_clientes "
                        + "where idCliente like '" + txtCriterio.getText() + "%' and estado order by 1";
            }
        }
        ResultSet rs = null;
        //ResultSet rs = Datos.getConsulta(sql);
        try {
            while (rs.next()) {
