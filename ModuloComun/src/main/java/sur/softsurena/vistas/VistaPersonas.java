...
public final class VistaPersonas extends javax.swing.JInternalFrame {
    ...
    private JTextFieldDateEditor v_editor;
    ...
    public VistaPersonas(Entidades entidades) {
        ...
        initComponents();
        btnNuevaPersona.setName("btnNuevaPersona2");
    ...
    private void initComponents() {
        ...
        javax.swing.ButtonGroup btnGMovilTelefono = new javax.swing.ButtonGroup();
        txtCedula1 = new javax.swing.JFormattedTextField();
        javax.swing.ButtonGroup btnGTipoPersona = new javax.swing.ButtonGroup();
        ...
        jsCantidadFilas.setToolTipText("Cantidad de Registros");
        jsCantidadFilas.setBorder(null);
        ...
        jsPaginaNro.setToolTipText("Numero Pagina");
        jsPaginaNro.setBorder(null);
        ...
        btnNuevaPersona.setContentAreaFilled(true);
    ...
    private void txtSNombreActionPerformed(ActionEvent evt) {//GEN-FIRST:event_txtSNombreActionPerformed
        if (txtPNombre.getText().isBlank() && !txtSNombre.getText().isBlank()) {
...
//                tblDireccion.getSelectedRow(), 3)).getId();
    ...
    private static void llenarTablaPersonas(Persona persona) {
        ...
        Predicate<Cliente> pCliente = (p) -> p.getIdPersona() > 0;
        Predicate<Empleado> pEmpleado = (p) -> p.getIdPersona() > 0;
        Predicate<Estudiante> pEstudiante = (p) -> p.getIdPersona() > 0;
        Predicate<Paciente> pPaciente = (p) -> p.getIdPersona() > 0;
        Predicate<Padre> pPadre = (p) -> p.getIdPersona() > 0;
        Predicate<Proveedor> pProveedor = (p) -> p.getIdPersona() > 0;
        if (jcbMostrarGenerico.isSelected()) {
            pCliente = (p) -> p.getIdPersona() >= 0;
            pEmpleado = (p) -> p.getIdPersona() >= 0;
            pEstudiante = (p) -> p.getIdPersona() >= 0;
            pPaciente = (p) -> p.getIdPersona() >= 0;
            pPadre = (p) -> p.getIdPersona() >= 0;
            pProveedor = (p) -> p.getIdPersona() >= 0;
        }
        ...
        if (jcbFiltroClientes.isVisible()) {
            if (jcbFiltroClientes.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Cliente.select(
                        ...
                        cliente -> {
                            Object[] bPersona = buscarPersona(cliente, "Cliente");
        ...
        if (jcbFiltroEmpleados.isVisible()) {
            if (jcbFiltroEmpleados.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Empleado.select(
                        ...
                        empleado -> {
                            Object[] bPersona = buscarPersona(empleado, "Empleado");
                            dtmPersonas.addRow(bPersona);
//                            if (!Objects.isNull(bPersona[3])) {
//                                dtmPersonas.addRow(bPersona);
//                            }
        ...
        if (jcbFiltroEstudiantes.isVisible()) {
            if (jcbFiltroEstudiantes.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Estudiante.select(
                        ...
                        estudiante -> {
                            Object[] bPersona = buscarPersona(estudiante, "Estudiante");
                            if (!Objects.isNull(bPersona[3])) {
                                dtmPersonas.addRow(bPersona);
                            }
                        }
        ...
        if (jcbFiltroPacientes.isVisible()) {
            if (jcbFiltroPacientes.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Paciente.select(
                        ...
                        paciente -> {
                            Object[] bPersona = buscarPersona(paciente, "Paciente");
                            if (!Objects.isNull(bPersona[3])) {
                                dtmPersonas.addRow(bPersona);
                            }
                        }
        ...
        if (jcbFiltroPadres.isVisible()) {
            if (jcbFiltroPadres.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Padre.select(
                        ...
                        padre -> {
                            Object[] bPersona = buscarPersona(padre, "Padre");
                            if (!Objects.isNull(bPersona[3])) {
                                dtmPersonas.addRow(bPersona);
                            }
                        }
        ...
        if (jcbFiltroProveedores.isVisible()) {
            if (jcbFiltroProveedores.isSelected() || jcbFiltroTodos.isSelected()) {
                M_Proveedor.select(
                        ...
                        proveedor -> {
                            Object[] bPersona = buscarPersona(proveedor, "Proveedor");
                            if (!Objects.isNull(bPersona[3])) {
                                dtmPersonas.addRow(bPersona);
                            }
                        }
    ...
    private static Object[] buscarPersona(Persona persona, String relacion) {
        var registro = new Object[TITULOS_PERSONA.length];

        registro[0] = relacion;
        registro[1] = persona.getGenerales();
        registro[2] = String.valueOf(
                persona.getPersona()
        ).equalsIgnoreCase("j") ? "JURÍDICA" : "FÍSICA";
        registro[3] = persona;
        registro[4] = persona.getSexo().equals('M') ? "MASCULINO" : "FEMENINO";
    ...
    public synchronized static void llenarTablaDirreciones(Integer idPersona) {
        if (Objects.isNull(idPersona)) {
            idPersona = VistaPersonas.idPersona;
        }
    ...
    private void registroEntidades(Integer id) {
        var cliente = Cliente.builder().idPersona(id).build();
        if (jcbCliente.isSelected()) {
        ...
        var empleado = Empleado.builder().idPersona(id).build();
        if (jcbEmpleado.isSelected()) {
        ...
        var estudiante = Estudiante.builder().idPersona(id).build();
        if (jcbEstudiante.isSelected()) {
        ...
        var paciente = Paciente.builder().idPersona(id).build();
        if (jcbPaciente.isSelected()) {
        ...
        var padre = Padre.builder().idPersona(id).build();
        if (jcbPadre.isSelected()) {
        ...
        var proveedor = Proveedor.builder().idPersona(id).build();
        if (jcbProveedor.isSelected()) {
    ...
    private void insertarDirecciones(Integer idPersona) {
        for (int i = 0; i < tblDireccion.getRowCount(); i++) {
            var direccion = M_ContactoDireccion.insert(
                    ContactoDireccion
                            .builder()
                            .idPersona(idPersona)
                            .idProvincia(
    ...
    private void insertarTelefonos(Integer idPersona) {
        for (int i = 0; i < tblTelefonos.getRowCount(); i++) {
            var telefono = M_ContactoTel.insert(
                    ContactoTel
                            .builder()
                            .idPersona(idPersona)
                            .telefono(
            ...
            if (!telefono.getEstado()) {
                mensajeResultado(telefono);
                break;
            }
        }
    }

    private void agregarTelefono(boolean porDefecto) {
        if (porDefecto) {
    ...
    private void insertarCorreos(Integer idPersona) {
        for (int i = 0; i < tblCorreos.getRowCount(); i++) {
            var correo = M_ContactoEmail.insert(
                    ContactoEmail
                            .builder()
                            .idPersona(idPersona)
                            .email(
            ...
            if (!correo.getEstado()) {
                mensajeResultado(correo);
                break;
            }
        }
    }

    private void agregarCorreo(boolean porDefecto) {
        if (porDefecto) {
